...
FRAMES_PER_BUFFER = int(TARGET_SAMPLE_RATE / 10)
...
def get_input_device_list():
    ...
    try:
        ...
        default_loopback = next((mic for mic in soundcard.all_microphones(include_loopback=True)
...
def _find_device_on_list(device, device_list):
    ...
    """
    return next((i for i, e in enumerate(device_list) if device and device.name == e[0].name), 0)


class StreamReader:
    """Class responsible for reading audio stream data from device"""
    def __init__(self, device, callback=None, clipping_callback=None,
                 level_callback=None, volume_getter=None):
        """
    ...
    def initialize(self):
        ...
        self._stop.clear()
        try:
            self._stream = self._device.recorder(TARGET_SAMPLE_RATE, channels=TARGET_CHANNELS)
