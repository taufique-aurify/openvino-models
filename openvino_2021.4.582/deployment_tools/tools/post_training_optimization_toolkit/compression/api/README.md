...
- `Engine` is responsible for model inference and provides statistical data and accuracy metrics for the model.
  ...
  >           `<POT_DIR>/compression/engines/ie_engine.py`, where `<POT_DIR>` is a directory where the Post-Training Optimization Tool is installed.
  >           It is based on the [OpenVINO™ Inference Engine Python* API](https://docs.openvinotoolkit.org/latest/_inference_engine_ie_bridges_python_docs_api_overview.html#api_reference)
...
## Use Cases
Before diving into the Python* POT API, it is highly recommended to read [Best Practices](../../docs/BestPractices.md) document where various 
...
- POT does not support the model in the [Simplified Mode](../../docs/BestPractices.md) or produces the optimized model with low 
accuracy in this mode.
- You already have the Python* script to validate the accuracy of the model using the [OpenVINO™ Inference Engine](https://docs.openvinotoolkit.org/latest/_docs_IE_DG_Deep_Learning_Inference_Engine_DevGuide.html).  
...
```
IEEngine is a helper which implements Engine class based on [OpenVINO™ Inference Engine Python* API](https://docs.openvinotoolkit.org/latest/_inference_engine_ie_bridges_python_docs_api_overview.html#api_reference).
...
- The model was converted to the OpenVINO™ Intermediate Representation (IR) from the source framework using [Model Optimizer](https://docs.openvinotoolkit.org/latest/_docs_MO_DG_Deep_Learning_Model_Optimizer_DevGuide.html).
...
model using the [OpenVINO™ Inference Engine](https://docs.openvinotoolkit.org/latest/_docs_IE_DG_Deep_Learning_Inference_Engine_DevGuide.html),
...
and available in the `<POT_DIR>/compression/api/samples` directory. There are currently five samples that demonstrate the implementation of `Engine`, `Metric` and `DataLoader` interfaces for classification, detection and segmentation tasks.
