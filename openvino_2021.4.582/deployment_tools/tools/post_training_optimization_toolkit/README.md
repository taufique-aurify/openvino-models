...
* [API](compression/api/README.md) that helps to apply optimization methods within a custom inference script written with OpenVINO Python* API.
...
* **[Python* API](compression/api/README.md)**: it allows integrating optimization methods implemented in POT into
...
Note: POT also supports optimization in the so-called [**Simplified mode**](@ref_pot_configs_README_simplified_mode) which is essentially a local
